...
ダッシュボードに登録済みのゲームを編集・削除する機能を実装する。

## 背景

- ユーザーがプレイ時間や購入金額を更新したい場合がある
- 間違って登録したゲームを削除したい場合がある
- ステータス変更（プレイ中→クリア済み等）のニーズがある
...
- [ ] 編集モーダルコンポーネント作成
- [ ] 既存データのプリフィル
- [ ] `updatePortfolioEntry` Server Action実装
- [ ] 楽観的UI更新（任意）

### 2. 削除機能

- [ ] 削除確認ダイアログ
- [ ] `deletePortfolioEntry` Server Action実装
- [ ] 削除後のリスト更新
...
```typescript
// src/app/actions/portfolio.ts に追加
...
- [ ] 編集ボタンクリックで編集モーダルが開く
- [ ] 既存データがフォームにプリフィルされている
- [ ] 購入金額、プレイ時間、ステータスを変更できる
- [ ] 更新後、ダッシュボードに反映される
- [ ] 削除ボタンクリックで確認ダイアログが表示される
- [ ] 削除後、ゲームがリストから消える
- [ ] 他ユーザーのデータは編集・削除できない（RLS）
- [ ] 未ログイン時は操作できない（エラー表示）

## デザイン仕様

- 編集モーダル: 登録モーダルと同じスタイル
- 削除確認ダイアログ: 赤色の削除ボタン
- ローディング状態: ボタンを無効化して「処理中...」表示
- エラー表示: 赤色テキストでユーザーに通知
