...
- 日次での自動データ更新
- OpenCritic API + RAWG APIからハイブリッドデータ取得
- エラーハンドリングとログ記録

## タスク一覧

### 1. Supabase Edge Functions 実装
- [x] 更新用 Edge Function の作成（`sync-games`）
- [x] OpenCritic API 連携（60件、3回バッチ）
- [x] RAWG API 連携（説明文・ジャンル補完）
- [x] データベース更新ロジック（全削除→再投入）

### 2. Supabase Cron Jobs 設定
- [x] 日次実行スケジュールの設定（毎日午前3時 JST）
- [x] 実行時間の最適化（UTC 18:00 = JST 03:00）

### 3. エラーハンドリング
- [x] エラーログの記録（operation_logsテーブル）
- [ ] リトライ機構の実装（将来的な改善として）
- [ ] アラート通知（オプション、将来的な改善として）

### 4. 監視とログ
- [x] 実行ログの記録（operation_logs）
- [x] 成功/失敗の統計（SQLクエリで確認可能）
- [x] ドキュメント作成（`docs/自動更新システム.md`）
