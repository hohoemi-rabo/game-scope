...
4. **operation_logs** - 操作ログ
   - **operation_type** (TEXT): 操作種別（`auto_sync`等）
   - **status** (TEXT): ステータス（`success` / `error`）
   - **message** (TEXT): 人間が読めるメッセージ（UI表示用）
   - **details** (JSONB): 構造化された詳細情報（デバッグ・分析用）
   - **created_at** (TIMESTAMPTZ): 作成日時（**タイムゾーン付き必須**）
   - 自動更新の実行記録、エラートラッキング
   - ヘッダーに最新ログのステータスを表示
...
- モバイル対応: 768px未満ではアイコンのみ表示

**自動更新ステータス表示**:
- `operation_logs`テーブルから最新の同期ログを取得
- 成功時: 🟢 緑ドット + "同期成功" + 経過時間
- 失敗時: 🔴 赤ドット + "同期失敗" + 経過時間
- 経過時間の表示: "最近" (1時間未満), "N時間前" (24時間未満), "N日前"
- `message`カラム: 人間が読めるメッセージ（UI表示用）
- `details`カラム: JSON形式の詳細情報（デバッグ用）

**重要**: `operation_logs.created_at`は`timestamptz`型を使用（タイムゾーン対応必須）
